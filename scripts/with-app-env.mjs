...
function main(argv) {
  ...
  const nodeDir = process.platform === "win32" ? dirname(process.execPath) : "";
  env.PATH = [nodeDir, localBin, env.PATH || ""].filter(Boolean).join(delimiter);
  // On Windows, `spawn` with `shell: true` invokes `cmd.exe` which does not
  // always inherit the parent's PATH in a usable way (e.g. when the wrapper
  // is launched by a tool that assembles a minimal env). For any command that
  // is itself `node`, run it through the current `node` binary using its
  // absolute path so the child doesn't depend on PATH lookup at all.
  let actualCommand = command;
  let actualArgs = args;
  if (process.platform === "win32" && /(^|[\\/])node(\.exe)?$/i.test(command)) {
    actualCommand = process.execPath;
    actualArgs = [command, ...args];
  }
  const child = spawn(actualCommand, actualArgs, { stdio: "inherit", env, shell: true });
