import { createBrowserHistory, parseHref } from "./@tanstack_history.js";
	...
	*/
	constructor(options, getStoreConfig) {
		...
		this.update = (newOptions) => {
			...
			this.isServer = this.options.isServer ?? void 0 ?? typeof document === "undefined";
		...
		this.load = async (opts) => {
			if (this.isServer) return loadServerRoute(this, opts);
			this.updateLatestLocation();
		...
		this.startViewTransition = (fn) => {
			...
			if (shouldViewTransition && !(typeof document === "undefined") && typeof document.startViewTransition === "function") {
		...
		this.resolveRedirect = (redirect) => {
			...
			if (redirect.options.href && isDangerousProtocol(redirect.options.href, this.protocolAllowlist)) throw new Error(`Redirect blocked: unsafe protocol in href "${redirect.options.href}". Allowed protocols: ${Array.from(this.protocolAllowlist).join(", ")}.`);
...
var normalize$2 = (str) => str.endsWith("/") && str.length > 1 ? str.slice(0, -1) : str;
function comparePaths(a, b) {
	return normalize$2(a) === normalize$2(b);
}
/**
...
async function awaitPendingMinimum(router, tx) {
	...
	let timer;
	try {
		await waitFor$1(new Promise((resolve) => {
...
async function runBackground(router, tx, base, tasks, settlement) {
	...
	if (isControl(reduced)) {
		transferMatchResources(router, next);
		if (reduced[0] === REDIRECTED$1 && router._tx === tx && router._committed === base) await followRedirect(router, tx, reduced);
		return;
	}
	await projectLane$1(router, reduced, tx[0].signal);
...
async function runClientTransaction(router, tx, forceStaleReload, onReady, sync, resolvedPrefix) {
	...
	if (isControl(result)) {
		const follow = result[0] === REDIRECTED$1 && router._tx === tx;
...
async function hydrate(router) {
	if (!window.$_TSR) throw new Error("Invariant failed: Expected to find bootstrap data on window.$_TSR, but we did not. Please file an issue!");
	...
	if (!dehydratedRouter) throw new Error("Invariant failed: Expected to find a dehydrated data on window.$_TSR.router, but we did not. Please file an issue!");
	...
	try {
		await waitFor$1(router.options.hydrate?.(dehydratedRouter.dehydratedData), controller.signal);
		if (!isCurrent()) return;
	...
	const retryFrom = (index) => {
		...
		for (const match of removed) if (getRoute$1(router, match).options.loader && (match.status === "success" || !match.invalid && "loaderData" in match)) cacheLoaderMatch(router, {
			...match,
			status: "success",
			error: void 0,
			preload: true
	...
	for (let index = 0; index < shared; index++) {
		...
		const route = getRoute$1(router, candidate);
	...
	let chunkFailure = 0;
	try {
		while (chunkFailure < chunks.length && await waitFor$1(chunks[chunkFailure], controller.signal)) chunkFailure++;
	} catch {
		return;
	}
	if (!isCurrent()) return;
	...
	await projectLane$1(router, [location, candidates], controller.signal, 0, verifiedAssetEnd);
	if (!isCurrent()) return;
...
//#endregion
//#region node_modules/@tanstack/router-core/dist/esm/isServer/development.js
var isServer = void 0;
//#endregion
export { isResolvedRedirect as $, interpolatePath as A, decodePath as B, encode as C, storageKey as D, setupScrollRestoration as E, trimPathLeft as F, hasOwn as G, escapeHtml as H, trimPathRight as I, isPlainArray as J, isDangerousProtocol as K, DEFAULT_PROTOCOL_ALLOWLIST as L, removeTrailingSlash as M, resolvePath as N, cleanPath as O, trimPath as P, isRedirect as Q, buildDevStylesUrl as R, decode as S, getElementScrollRestorationEntry as T, functionalUpdate as U, deepEqual as V, hasKeys as W, replaceEqualDeep as X, isPlainObject as Y, dehydrateSsrMatchId as Z, executeRewriteInput as _, hydrate as a, parseSearchWith as b, SearchParamError as c, getLocationChangeInfo as d, parseRedirect as et, lazyFn as f, composeRewrites as g, createNonReactiveReadonlyStore as h, _getRenderedMatches as i, notFound as it, joinPaths as j, exactPathTest as k, defaultSerializeError as l, createNonReactiveMutableStore as m, loadServerRoute as n, rootRouteId as nt, PathParamError as o, trailingSlashOptions as p, isModuleNotFoundError as q, _getAssetMatches as r, isNotFound as rt, RouterCore as s, isServer as t, redirect as tt, getInitialRouterState as u, defaultParseSearch as v, defaultGetScrollRestorationKey as w, stringifySearchWith as x, defaultStringifySearch as y, createControlledPromise as z };
