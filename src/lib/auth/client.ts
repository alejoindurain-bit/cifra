...
import { DEPLOYED_SIGN_OUT_TIMEOUT_MS, runPreSignInSignOut, runSignOut } from "../../../scripts/sign-out-plan.mjs";
import { SIGNED_OUT_KEY } from "../constants";
...
/** Keys that are app prefs, not auth — leave them alone on sign-out. */
const AUTH_STORAGE_KEEP = new Set([
  "cifra-hide-demo-logins",
  "cifra-generar-recibo",
  "cifra-hide-balances",
  SIGNED_OUT_KEY,
]);

const AUTH_STORAGE_RE = /auth|session|better-auth|grok-auth|estudio|last.?user|last.?estudio/i;
...
function sweepStorage(store: Storage): void {
  const keys: string[] = [];
  ...
  for (const key of keys) {
    if (AUTH_STORAGE_KEEP.has(key)) continue;
    if (AUTH_STORAGE_RE.test(key)) store.removeItem(key);
  }
}

function expireVisibleAuthCookies(): void {
  ...
  for (const part of raw.split(";")) {
    ...
    if (!name) continue;
    if (!/grok-auth|better-auth|session_token|session_data|account_data/i.test(name)) continue;
    const expired = `${name}=; Path=/; Max-Age=0; Expires=Thu, 01 Jan 1970 00:00:00 GMT`;
    document.cookie = `${expired}; Secure; SameSite=Lax`;
    document.cookie = expired;
  }
}

/** Drop bearer, auth storage, last-user/estudio leftovers, and JS-visible cookies. */
...
export async function signIn(
  ...
  // Force hasBearer so leftover email/password cookie sessions are invalidated
  // too (preview otherwise skips the server call when no bearer is stored).
...
/**
 * Drop any leftover session (cookie or bearer) without leaving the page.
 * Used right before a new email/password sign-in so switching estudios is clean.
 */
export async function dropLocalSession(): Promise<void> {
  await runPreSignInSignOut({
    ...
    requestSignOut: () => authClient.signOut(),
    clearToken: () => setBearerToken(null),
    // Email/password lives in the cookie. Give the server time to clear it
    // (preview default is 1.5s and often leaves the previous user).
    timeoutMs: DEPLOYED_SIGN_OUT_TIMEOUT_MS,
  });
  clearAuthStorage();
}
 ...
 *
 * Email/password sessions live in the cookie, not the bearer. The live-preview
 * plan skips the server call when `hasBearer` is false, so we force it true
 * here — otherwise "Cerrar sesión" leaves the cookie and /login bounces back.
 */
